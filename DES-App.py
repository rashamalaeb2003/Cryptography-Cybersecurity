...
# Setup dictionary to convert from Hexadecimal to Decimal
def hex2bin(s):
    mp = {'0': "0000",
          '1': "0001",
          '2': "0010",
...
# Setup dictionary to convert from Decimal to Hexadecimal
def bin2hex(s):
    mp = {"0000": '0',
          "0001": '1',
          "0010": '2',
...
# Permute function to rearrange the bits.Done after s-box
def permute(k, arr, n):
    permutation = ""
    for i in range(0, n):
...
# if a and b are similar adds a 0, if different adds a 1
def xor(a, b):
    ans = ""
...
final_perm = [40, 8, 48, 16, 56, 24, 64, 32,
 ...
 #Encrypt Plaintext as Hexadecimal using submitted Hexadecimal key 
def encrypt(pt, rkb, rk):
    pt = hex2bin(pt)
    
    # rk is the array of all round keys
    # rkb is 1 round key represented as 48 bits
 
    # Initial Permutation
    pt = permute(pt, initial_perm, 64)
    
 
    # Splitting 2 32-bit halves
    left = pt[0:32]  #Left PT
    right = pt[32:64] # Right PT
    
    # Go over 16 rounds of DES
    for i in range(0, 16):
        ...
        sbox_str = ""
        for j in range(0, 8):
            #Take the first and last bit as the row
            row = bin2dec(int(xor_x[j * 6] + xor_x[j * 6 + 5]))
            #Take the other digits as the column
            col = bin2dec(
                int(xor_x[j * 6 + 1] + xor_x[j * 6 + 2] + xor_x[j * 6 + 3] + xor_x[j * 6 + 4]))
            val = sbox[j][row][col] #Substitute using the correct S-box coordinates
            sbox_str = sbox_str + dec2bin(val)
        # Concatinate all the S-box outputs
        # Applying permutation P after S-box
        ...
        # Swaps Left and Right at every round except the last one
        if(i != 15):
            left, right = right, left
        
 
    # Combination of 64 bits
    combine = left + right
...
def on_encrypt():
    while True:
        ...
        if not rkb or not rk:
            ...
            break  # Break out of the loop if a valid key is provided
...
def perform_another_operation():
    ...
    if not answer:
        ...
        combobox_operation.delete(0, tk.END)
        show_main_widgets()
...
def show_decrypt_widgets():
    ...
    label_result1.configure(bg='#E6E6FA')
